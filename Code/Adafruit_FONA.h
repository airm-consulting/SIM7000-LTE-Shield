...
//   boolean MQTTreceive(const char* topic, const char* buf, int maxlen);
 ...
 protected:
  ...
  uint8_t getReply(FONAFlashStringPtr prefix, int32_t suffix, uint16_t timeout); // Don't set default value or else function call is ambiguous.
...
class Adafruit_FONA_LTE : public Adafruit_FONA {
 ...
 public:
  ...
  boolean MQTT_Init(const char* ca=NULL, const char* client_cert=NULL, const char* client_key=NULL);
  boolean MQTT_Startup();
  boolean MQTT_Connect(const char* username, const char* password, const char* serverAddr, uint16_t timeAlive, const char* clientId);
  ...
  char* readSingleLine(uint16_t timeout = (FONA_DEFAULT_TIMEOUT_MS*2));
  char* readMultipleLines(uint16_t timeout=(FONA_DEFAULT_TIMEOUT_MS*2));
  #endif
